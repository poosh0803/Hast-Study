...
// Wipes every imported evaluation — all batch notes and all per-set/
// per-question ones. Kept for a one-click full reset.
export function clearAllEvaluations() {
  writeStore({ batches: [], subjects: {} });
}

// Deletes specific items: `batchIndexes` are positions in the `batches`
// array (as returned by getAllEvaluations — the caller must use the
// original, non-reversed index), `entries` are
// { subject, setId, questionId } triplets (questionId is "_set" for a
// Write prompt's whole-piece note). Cleans up now-empty setId/subject
// objects afterward so the store doesn't accumulate empty shells.
export function deleteEvaluations({ batchIndexes = [], entries = [] } = {}) {
  const store = readStore();

  if (batchIndexes.length > 0) {
    const toRemove = new Set(batchIndexes);
    store.batches = store.batches.filter((_, i) => !toRemove.has(i));
  }

  for (const { subject, setId, questionId } of entries) {
    const setEntries = store.subjects[subject]?.[setId];
    if (!setEntries) continue;

    delete setEntries[questionId];
    if (Object.keys(setEntries).length === 0) {
      delete store.subjects[subject][setId];
    }
    if (Object.keys(store.subjects[subject]).length === 0) {
