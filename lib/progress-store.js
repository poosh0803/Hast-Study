...
// For Write (no `questions`, so `recordAnswer` doesn't apply) — saves the
// current draft text. Lives alongside `answers` in the same per-set record
// so a Write set and a quiz set share the same reset/export plumbing.
export function saveDraft({ subject, setId, text }) {
  const store = readStore();
  if (!store[subject]) store[subject] = {};
  if (!store[subject][setId]) store[subject][setId] = {};

  const wordCount = text.trim() ? text.trim().split(/\s+/).length : 0;
  store[subject][setId].draft = { text, wordCount, updatedAt: new Date().toISOString() };
  store[subject][setId].updatedAt = new Date().toISOString();
